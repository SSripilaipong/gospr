...
- Every ~2s each node gossips a snapshot (collection → document → state; singleton = doc `""`) to one random peer; the peer merges each document via that type's user-defined `merge` expr (e.g. `zip max` → elementwise max over the union of node slots). Trade-off: gossip carries EVERY materialized document each round and there is no delete API, so payload grows with total document count — fine at sandbox scale, the first thing to revisit for real deployments (delta/per-doc gossip).
...
| `node` | Lifecycle + message loop. All sends cross the `Peer` seam (prod `ChanPeer`) — the single interception point. **Documents:** every collection is a `colState{spec,key,docs map[doc]CRDT}`; a singleton is the reserved doc key `""` (created eagerly at Initialize), keyed collections start sparse. `resolveDocument` is the sole shape/canonicalization boundary; all internal paths (apply/query/merge/gossip/sync) take a `documentRef`, public singleton APIs are wrappers passing `""`. **Sync-quorum layer:** `ApplySync`/`ApplyDocumentSync` (apply→`pushQuorum`) / `QuerySync`/`QueryDocumentSync` (ABD two-phase); quorum rule `reached` = `holders/N > ratio` (strict, so `0.5` is a true majority; `ratio ≥ AllRatio` = all N); unmet → `ErrQuorumUnreached` (503). |
| `gateway` | HTTP: deploy / apply / query + Swagger regen; keyed collections add 3-segment `{collection}/{document}/{action\|query}` routes (segment count disambiguates, no mux conflict). `parseSync` reads the `X-Gospr-Sync-Ratio` header; maps `ErrQuorumUnreached`→503, other errors→400. |
...
builder/
  builder.go          Build(Plan) → BuiltPlan{Models, Functions, Collections, Fingerprint}: orchestration, Model/CollectionSpec, exhaustive query-result-metadata switch (vkStruct/vkNum/vkBool/vkString, default = internal error) + validators (validateFnParams allows vector params + resolveScalarParams(allowString split): update params may be strings, query params may not, rejects structs); missing-merge check / validateResolvedProgram gate / per-model `prover.Prove` / Funcs attach all iterate types.vectorOrder (declaration order — deterministic first error); BuiltCollection.Key (*ParamSpec, nil=singleton) validated+normalized via resolveScalarParams(allowString=true)
  typereg.go          typeReg (struct/alias registries + vector Models + typeOrder/structOrder/aliasOrder/vectorOrder declaration-order slices; maps are lookup-only) ; resolveTypes dispatches through typeOrder; resolveToken accepts `string` leaf, rejects vector names; reg.isVector; on-demand resolveStruct/resolveVectorElem/resolveAlias sharing one cycle set
  resolve.go          env.resolve (Name→Var/Ref, StructLit/Field, `reduce fn init vec`, `write` combinator) + arityOf; validateResolvedProgram/validateResolvedExpr — the post-Build invariant walker (exhaustive over ExprKind: leaves must be Var/Ref/literal; surviving ExprName, missing required children, unknown kinds → `internal:` errors), runs on every fn/merge/query/update body in source order before prover.Prove
  checker.go          checker (vtype{kind,num,fields,fn,elem}+vElem/vVector, vkFunc/vkVector via isFunc, resolveTokenVtype (vector param/return, distinct from leaf-only resolveToken), per-op rules + numBin, applyArgs (cmpOps accept num/num OR str/str via checkCmpOperands, reject mixed)/resultOf, subVtype/unify/vtypeEqual (struct/function/vector structural), typeOf→typeOfGuards/typeOfReduce(folds the vec arg's element)/checkQueryFn(query = fn-of-vector, containsVector gate)/checkCombinatorFn(write applies fn to the vector), annotation-seeded+strict inferReturn); paramVtype/paramScope/elemTOf return `internal:` errors on broken invariants — no silent rat fallback
  builder_test.go     hard-coded-AST integration test + error/duplicate/fn/arity/type + numeric-subtype + convergence-rejection cases; declaration-order tests, walker table tests (unresolved name in every child position + malformed/unknown kinds), invariant-helper error tests
...
node/
  node.go             lifecycle + message loop; Initialize (stores Fingerprint), PropagatePlan; Peer/ChanPeer (done-guarded), AddPeer(id,p)+peerByID, Done(), MessageKind, Initialized(), Stop(). Documents (design in the `node` row above): colState/documentRef, resolveDocument→normalizeDocumentKey/canonicalDecimal (decimal-only canonical keys, no `/`), document() (RLock fast path; miss = fresh UNregistered zero-state CRDT for reads), apply/mergeSnapshot/mergeCollection materialize ON SUCCESS ONLY (see Gotchas), Apply/Query + *Document* variants, Snapshot/SnapshotWireAll/MergeSnapshot are collection→doc two-level (snapshotEmpty skips empty gossip docs). Synchronous-quorum symbols: SyncPush/Pull/AckMsg (+Document field), WithSyncTimeout, ErrQuorumUnreached, pendingReq registry (register(reqID,doc)/nextReqID), handleSync*/routeAck demux (ack doc-scope checked after dup check), reached/collectAcks/pushQuorum/pullQuorum/broadcast, Apply/QuerySync + *DocumentSync, AllRatio, mergeCollection/snapshotCollection/ValidateQuery/ValidateDocumentQuery. Invariants: collectAcks counts a pull ack only after its merge succeeds; an empty pull/push on an unmaterialized doc succeeds WITHOUT materializing
  node_test.go        Stop() halts gossip + is idempotent; MessageKind
  linearize_test.go   sync write/read, quorum failure (+ctx-cancel), ack validity (undeployed + fingerprint mismatch), zero-ack short-circuit, read preflight, ChanPeer done-bound, DTO gob/json codec, distinct-peer ack counting (no inflation / no crowding-out / failed-merge pull ack doesn't count / wrong-doc-scope ack dropped); keyed-collection tests: sparse canonical docs, failed apply/merge + empty merge don't materialize, concurrent first-touch, sync untouched-query + replication, gossip doc scoping
...
sandbox/             observe/interact playground; imports only parser/builder/node/crdt — designed to lift into its own repo. `gospr sandbox run`
  ...
  server.go           HTTP+SPA: GET state/events(SSE), POST deploy(pin-the-plan, 409 on 2nd, zero-collection guard)/reset/links/speed, POST/GET nodes/{id}/collections/{collection}[/{document}]/{action|query} (string-only params); parseSync+writeOpError → Apply/QuerySync (+*Document* when the segment is present) under the cluster RLock, ErrQuorumUnreached→503; wireToState splits doc "" → nodes[].collections (legacy shape) vs keyed docs → nodes[].documents; planSchema carries collectionSchema.Key; //go:embed web/dist
  *_test.go           network/peer(done-abort)/cluster(reset + reset-reconnect-atomic-with-deploy race) + linearize_test(partitioned linearized update→503) unit tests
  web/                Vite+TS+Web Components SPA (no framework). App-shell layout (topbar + focal graph + right inspector). src/components/{sandbox-app(orchestrator: app-shell markup; polls /state ~750ms; owns selected node + selectedCollection + liveQuery + lastDeployedSource; /events SSE used only for Reset),node-graph(SVG circle; shows ONLY the selected collection; per-node value chip + optional live-query chip; partition by clicking links; node blinks on state change),node-panel(the right inspector: cluster-wide live-label query picker + selected-node invoke blocks + network delay),deploy-modal(the only DSL editor — opened from the topbar)}; dist/ committed + embedded
                      Concepts: ONE collection shown at a time (topbar selector drives the graph + inspector). KEYED collections are API-only in the SPA (liveLabelQueries excludes them; node-panel shows a notice) — nodes[].documents is in the state payload but not rendered. A LIVE LABEL polls a (param-less) query across every node each tick and renders the result as a chip under each node (queryAll uses Promise.allSettled + the app skips nodes lacking the collection, so a not-yet-deployed/partitioned node never breaks the poll). Deploy is one-shot per cluster (server pins the plan); the modal redeploys by VALIDITY-PREFLIGHT: POST /deploy first — 400 = bad code (cluster untouched), 409 = valid but pinned → only then reset() + redeploy. Server stores no DSL source, so the modal prefills the last same-session source (else SAMPLE).
...
swagger/
  swagger.go          Generate(BuiltPlan) → OpenAPI 3.0 JSON; type-switches on *builder.Model; numSchema → string (numbers are exact-rational strings on the wire) with the NumType named in the description; structSchema → object schema for a struct query result (Method.ResultStruct); a keyed collection's paths get a `{keyName}` segment + required path parameter (documentKeyParam: string schema, decimal-syntax note for numeric keys)
  swagger_test.go

gateway/
  gateway.go          HTTP: POST /api/cluster/deploy, POST /api/collections/{collection}[/{document}]/{action},
                      GET /api/collections/{collection}[/{document}]/{query}?params=... (passed as exact-rational strings),
                      GET /api/swagger.json, GET /api/docs (Swagger UI). Empty PathValue("document") → singleton node API, else *Document* variant. parseSync (presence-based X-Gospr-Sync-Ratio header: absent→async, present→sync at fraction in [0,1) or `all`; empty/≥1/NaN/Inf/range→400) + writeOpError (ErrQuorumUnreached→503) dispatch update/query to node.Apply/QuerySync
...
parser/
  types.go            AST: ElemType{Kind,Elem,Inner,Fields} (ElemKind = KindStruct|KindVector(Elem token or Inner inline struct)|KindElemRef(`V.Elem` alias)), Expr sum type (ExprKind incl. StrLit/Guards/Reduce(+Vec)/Zip/Local/**Write**), GuardCase, ValType, RefKind, TypeDef/FnDef(+RetType: optional `-> type` token)/MergeDef/QueryDef/UpdateDef/CollectionSpec(+Key *ParamSpec: optional `[name::type]` document key), Plan
...
collection Users[id::string] = T # KEYED collection: sparse independent documents addressed by key
                                #   key type: string or a numeric scalar (aliases normalized); routing metadata ONLY —
                                #   never stored in slots, so zero prover/checker impact
...
- **Keyed collections** (`collection C[k::t] = T`): each document is an independent CRDT of type T, materialized lazily. Key values canonicalize to **decimal-only** spellings (`canonicalDecimal` in `node` — `0.50`→`0.5`, `05`→`5`; `1/2`/`1e2` rejected, so a URL segment never needs a `/`; a non-finite-decimal rational like 1/3 is unaddressable, matching source-literal syntax). String keys: non-empty, valid UTF-8, no `/`. HTTP routes gain a `{document}` segment (`/api/collections/{collection}/{document}/{action|query}`); the legacy 2-segment route on a keyed collection (or a document on a singleton) is a 400.
...
- **New expression form**: add/realize an `ExprKind`/`ElemKind` in `parser/types.go`, parse it in `dsl.go`, resolve it in `builder.env.resolve`, type-check it in `checker.typeOf`, add a case to `validateResolvedExpr` (`builder/resolve.go` — its default rejects unknown kinds, so a new kind fails Build until handled), and evaluate it in `crdt/vector.go` (`eval`). Struct vectors (`ExprStructLit`/`ExprField`) are the worked example — grep those kinds across the layers to see the full seam list.
...
- **Name resolution invariant:** `ExprName` appears ONLY in parser output; after `Build`, every leaf is an `ExprVar` (bound param), `ExprRef` (symbol, with `Arity`/`RefKind`), or a literal (`ExprNumLit`/`ExprStrLit`). Proof/optimization passes can rely on this — no unresolved leaves in a `*Model` or `Functions` entry. Enforced at the end of `Build` by `validateResolvedProgram` (`builder/resolve.go`), which walks every emitted body before `prover.Prove`.
...
- **Documents materialize ONLY on success** (failed/empty ops leave no trace — else one bad request would create an empty doc that gossip replicates cluster-wide forever, with no delete API). Three sites in `node`: `apply` runs the FIRST touch under the write lock (create+Apply+publish atomically — an update can NOT be applied to a temp and merged into a concurrent winner, two Adds would collapse under `max`); `mergeSnapshot`/`mergeCollection` merge into a temp and publish on success (a concurrent winner IS safe to merge into — merge is commutative/idempotent); `MergeSnapshot` also skips empty gossiped docs (`snapshotEmpty`). Reads never materialize: `document()` returns a fresh unregistered CRDT on miss.
